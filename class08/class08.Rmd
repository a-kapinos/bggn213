...
>Q. Complete the following code to find out how many rows and columns are in x?
___(x)
...
If we remove the beside=T argument, then the bars will not be plotted besides one another. See below.
...
Ask: as in which values are different for N. Ireland? Or just that N. Ireland is the most unique (ie. had the most values that deviate significantly/exhibit values that do not fall along the diagonal)
...
Along PC1 we can go in the positive or negative direction. Comparing this plot to the 
...
We can make this a bit more useful and attractive by updating the script
```{r}
